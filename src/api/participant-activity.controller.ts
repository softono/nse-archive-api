...
export async function getParticipantActivity(
  ...
  const clientType = req.query.clientType
    ? String(req.query.clientType)
    : undefined;

  if (!metric || (metric !== "oi" && metric !== "volume") || !from || !to) {
    res
      .status(400)
      .json({ error: "metric ('oi' or 'volume'), from, and to are required" });
    return;
  }
  ...
  if (clientType)
    conditions.push(eq(participantActivity.clientType, clientType));
  ...
  const rows = await db
    ...
    .orderBy(
      asc(participantActivity.tradeDate),
      asc(participantActivity.clientType),
    );
