...
async function fetchJsonAndArchive(
  source: string,
  path: string,
): Promise<unknown> {
  const res = await fetchNseJson(path);
  const text = await res.text();
  await archiveRawFile({
    source,
    tradeDate: todayStr(),
    ext: "json",
    data: text,
...
export async function fetchSurveillanceLists(): Promise<{
  asm: unknown;
  gsm: unknown;
} | null> {
  try {
